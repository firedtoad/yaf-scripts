...
class Afx_Db_Adapter
{
    ...
    private function _initConnection(){
      ...
      self::$write_dsn= 'mysql:host=' . $master['host'] . ';port=' .$master['port'] . ';dbname=' . $master['dbname'].';charset='.$master['charset'].';';
        ...
        try{
          self::$link_read = new PDO(self::$read_dsn, $slave['user'],$slave['password'],array(PDO::ATTR_TIMEOUT=>1,PDO::ATTR_PERSISTENT=>1));
          self::$link_write = new PDO(self::$write_dsn, $master['user'],$master['password'],array(PDO::ATTR_TIMEOUT=>1,PDO::ATTR_PERSISTENT=>1));
