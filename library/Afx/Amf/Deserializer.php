...
/**
 * Afx_Amf_Deserializer takes the raw amf input stream and converts it PHP objects
...
class Afx_Amf_Deserializer implements Afx_Amf_Common_IDeserializer{
    ...
    protected function readHeaders() {
        ...
        while ($this->headersLeftToProcess--) { // loop over all of the header elements
            ...
            $header = new Afx_Amf_Header($name, $required, $content);
    ...
    public function readData($type) {
        switch ($type) {
            ...
            case 5: // null
                return null;
            case 6: // undefined
                return new Afx_Amf_Types_Undefined();
    ...
    protected function readDouble() {
        ...
        $this->currentByte += 8;
        if (Afx_Amf_Util::isSystemBigEndian()) {
            $bytes = strrev($bytes);
        }
    ...
    protected function readDate() {
        ...
        $date = new Afx_Amf_Types_Date($ms);
        return $date;
    }
    ...
    protected function readXml() {
        ...
        return new Afx_Amf_Types_Xml($str);
    }
     ...
     * to the object Afx_Amf_Constants::FIELD_EXPLICIT_TYPE.  This property will be overwritten if it existed within the class already.
     *
    ...
    protected function readAmf3Xml() {
        ...
        return new Afx_Amf_Types_Xml($xml);
    }
    ...
    protected function readAmf3ByteArray() {
        ...
        $handle = $handle >> 1;
        if ($inline) {
            $ba = new Afx_Amf_Types_ByteArray($this->readBuffer($handle));
    ...
    protected function readAmf3Object() {
        ...
        if($classDefinition['externalizable']){
            $externalizedDataField = Afx_Amf_Constants::FIELD_EXTERNALIZED_DATA;
