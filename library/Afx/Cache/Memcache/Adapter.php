<?php
class Afx_Cache_Memcache_Adapter implements Afx_Cache_Adapter
{
    ...
    public function init ()
    {
        ...
        foreach ($this->__servers as $value)
        {
            ...
            $mem->addServer($value['host'], $value['port'], $value['persist'], $value['weight'], $value['timeout']);
    ...
    {
        $value = Afx_Common::cachePreUpdate($value, $this->__cache[$key]);
        $retval = $this->__mem->replace($key, $value, $flag, $expire);
        if ($retval)
        {
    ...
    {
        $value = Afx_Common::cachePreUpdate($value, $this->__cache[$key]);
        $retval = $this->__mem->cas($key, $value, $flag, $expire);
        if ($retval)
        {
            $this->__cache[$key] = $value;
        }
    ...
    {
        $ret = $this->__mem->add($key, $value, $flag, $expire);
        if ($ret)
        {
            $this->__cache[$key] = $value;
        }
    ...
    {
        $ret = $this->__mem->set($key, $value, $flag, $expire);
        if ($ret)
        {
            $this->__cache[$key] = $value;
        }
